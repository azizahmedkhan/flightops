            ...
            async with websockets.connect(f"{self.ws_url}/ws/{session_id}/{client_id}") as websocket:
                ...
                for message in messages:
                    ...
                    try:
                        ...
                        self.results.add_response(10.0, False, "WebSocket timeout")
...
class TestLoadTesting:
    ...
    async def test_small_load(self):
        ...
        print(f"Small Load Test Results: {stats}")
        
        # Assertions for small load
        ...
        assert stats["requests_per_second"] > 1, f"Requests per second too low: {stats['requests_per_second']}"
    
    @pytest.mark.asyncio
    async def test_medium_load(self):
        """Test with medium load (100 concurrent users)"""
        tester = ChatbotLoadTester()
        tester.results.start_time = datetime.now()
        ...
        print(f"Medium Load Test Results: {stats}")
        ...
        assert stats["p95_response_time"] < 15.0, f"P95 response time too high: {stats['p95_response_time']}s"
    ...
    async def test_high_load(self):
        ...
        # Simulate 500 concurrent users for 120 seconds
        tasks = [
            tester.simulate_user_session(i, duration=120)
            for i in range(500)
        ]
        
        # Process in smaller batches for high load
        batch_size = 50
        for i in range(0, len(tasks), batch_size):
            batch = tasks[i:i + batch_size]
            await asyncio.gather(*batch, return_exceptions=True)
        ...
        print(f"High Load Test Results: {stats}")
        
        # More lenient assertions for high load
        assert stats["success_rate"] >= 70, f"Success rate too low: {stats['success_rate']}%"
        assert stats["avg_response_time"] < 20.0, f"Average response time too high: {stats['avg_response_time']}s"
        assert stats["p95_response_time"] < 30.0, f"P95 response time too high: {stats['p95_response_time']}s"
    
    @pytest.mark.asyncio
    async def test_extreme_load(self):
        """Test with extreme load (1000+ concurrent users)"""
        tester = ChatbotLoadTester()
        tester.results.start_time = datetime.now()
        
        # Simulate 1000 concurrent users for 180 seconds
        tasks = [
            tester.simulate_user_session(i, duration=180)
            for i in range(1000)
        ]
        
        # Process in very small batches for extreme load
        batch_size = 25
        for i in range(0, len(tasks), batch_size):
            batch = tasks[i:i + batch_size]
            await asyncio.gather(*batch, return_exceptions=True)
        ...
        print(f"Extreme Load Test Results: {stats}")
        
        # Very lenient assertions for extreme load
        assert stats["success_rate"] >= 60, f"Success rate too low: {stats['success_rate']}%"
        assert stats["avg_response_time"] < 30.0, f"Average response time too high: {stats['avg_response_time']}s"
        assert stats["p99_response_time"] < 60.0, f"P99 response time too high: {stats['p99_response_time']}s"
...
class TestStressTesting:
    ...
    async def test_burst_traffic(self):
        ...
        # Simulate sudden burst of 200 users all connecting at once
        burst_tasks = [
            tester.simulate_user_session(i, duration=10)
            for i in range(200)
        ]
        ...
        stats = tester.results.get_stats()
        print(f"Burst Traffic Test Results: {stats}")
        
        # Should handle burst gracefully
        assert stats["success_rate"] >= 80, f"Burst handling success rate too low: {stats['success_rate']}%"
    
    @pytest.mark.asyncio
    async def test_long_running_sessions(self):
        """Test long-running chat sessions"""
        tester = ChatbotLoadTester()
        
        # Simulate 50 users with very long sessions (5 minutes)
        long_tasks = [
            tester.simulate_user_session(i, duration=300)
            for i in range(50)
        ]
        ...
        stats = tester.results.get_stats()
        print(f"Long Running Sessions Test Results: {stats}")
        
        # Should maintain performance over time
        assert stats["success_rate"] >= 85, f"Long session success rate too low: {stats['success_rate']}%"
        assert stats["avg_response_time"] < 15.0, f"Long session response time too high: {stats['avg_response_time']}s"
